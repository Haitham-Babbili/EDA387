 ...
 * connections vector.
 */
static bool accept_connection(int listenfd, ConnectionData& connData);
...
#	endif
	...
	while( 1 )
	{
        ...
        for (size_t i = 0; i < connections.size(); i++)
        {
            ...
            {
                FD_SET(connData.sock, &readfds);
            } else {
                FD_SET(connData.sock, &writefds);
            }
            nfds = std::max(nfds, connData.sock);// create a list for file descriptor
        }
        
        //check if it's ready to read an write
        ...
        if (FD_ISSET(listenfd, &readfds))
        {
            ...
            memset(&connData, 0, sizeof(connData));// clear and reset the memory
            bool success = accept_connection(listenfd, connData);
            if (!success) {
...
#			endif
        ...
        // After estaplish the connection and it's open then start send and recive data
        for (size_t i = 0; i < connections.size(); i++) {
            ConnectionData& connData = connections[i];
            bool still_open = true;
            if (FD_ISSET(connData.sock, &readfds)) {
                still_open = process_client_recv(connData);
            } else if (FD_ISSET(connData.sock, &writefds)) {
                still_open = process_client_send(connData);
            }
            if (!still_open) {
                close(connData.sock);
                connData.sock = -1;
...
static bool process_client_send( ConnectionData& cd )
{
	...
	ssize_t ret = send( cd.sock, 
		...
		MSG_NOSIGNAL // for win/linux: MSG_NOSIGNAL,suppress SIGPIPE signals for mac:SO_NOSIGPIPE, generate EPIPE instead
	);
